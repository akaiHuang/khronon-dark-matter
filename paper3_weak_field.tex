...
\title{Dark Matter Phenomenology from Khronon Condensation\\
...
dimensional analysis, which reproduces the observed dark matter
abundance $\Omega_{\mathrm{DM}} = \delta_0(2{+}\delta_0)/3 = 0.265$
with CDM-like perturbations ($\cs^2 = 0$) \emph{without invoking
any dark matter particle}; and the
...
\emph{Three long-standing problems of MOND.}---Since Milgrom's
original proposal~\cite{Milgrom1983}, three fundamental questions
have remained unanswered:
\begin{enumerate}
\item \emph{Why $a_0 \approx cH_0$?}---The MOND acceleration
  scale $a_0 \approx 1.2 \times 10^{-10}\;\mathrm{m/s}^2$ is
  numerically close to $cH_0/(2\pi)$, a cosmological quantity.
  No MOND theory has explained this ``Milgrom coincidence.''
\item \emph{Why does the CMB look like $\Lambda$CDM?}---MOND
  modifies gravity at all scales, yet the CMB acoustic peaks
  are exquisitely fit by $\Lambda$CDM with pressureless cold dark
  matter.  Previous relativistic completions (TeVeS, AeST) require
  additional fields and parameters to reconcile this.
\item \emph{What determines $\mu(x)$?}---The MOND interpolating
  function $\mu(x)$ has been a free function for forty years,
  with multiple proposals but no theoretical selection principle.
\end{enumerate}
The $\Sigma = 2\ln Q$ framework addresses these three questions
through distinct mechanisms: $\mu_0 = H_0/c$ is the unique scale
without Planck physics, providing a \emph{theoretical} answer to
question~1; the no-go theorem (Theorem~\ref{thm:nogo}) forces
$\cs^2 = 0$ via the tensor field, providing a \emph{mathematical}
answer to question~2; and the Petz recovery bound constrains the
interpolating function to $\mu(x) = 1 - e^{-\sqrt{x}}$, providing
a \emph{testable candidate} for question~3---not a derivation, since
the deep-MOND asymptotic behavior (BTFR) is used as a boundary
condition, but the transition-region shape is a genuine prediction
verifiable against SPARC data (Sec.~\ref{sec:sparc_test}).
...
in which all dark matter phenomenology---flat rotation curves,
the CMB acoustic peaks, Bullet Cluster offsets, and the
matter power spectrum---emerges from the ghost condensation of
a Khronon field, \emph{without any new particle species}.
If this framework is correct, direct detection experiments
(LZ, XENONnT, PandaX) and collider searches should yield null
results for dark matter particles.
The theory contains a free function $J(Y)$ (the MOND interpolating
function), and two key assumptions (running $\mu$, the
$a_0$--$\mu_0$ coincidence) remain unproven.
...
\item \textbf{Modular flow}: if the Khronon is the modular flow
  ...
  gives $\mu(k) \sim k$.
\end{enumerate}
The quantum relative entropy monotonicity used here follows from~\cite{Lindblad1975,Uhlmann1977}, independently of the specific proof route discussed in~\cite{DorauMuch2025}.
...
\new\  The $\tauasym$ framework \emph{suggests} a specific MOND
interpolating function \emph{inspired by} the Crooks fluctuation
...
The precise mapping from Crooks statistics to the MOND interpolation
requires identifying $\Sigma$ with $\sqrt{g_{\mathrm{bar}}/a_0}$,
which remains a conjecture; the functional form is proposed as an
ansatz whose deep-MOND and Newtonian limits are correct by
construction.
...
If the Crooks ansatz is validated against the SPARC 175-galaxy
sample, this would eliminate the free function $J(Y)$ entirely,
making the theory's galactic predictions parameter-free (given
$a_0$).  This is the single most impactful testable prediction of
the $\tauasym$ framework for galactic phenomenology.

\subsection{Testable prediction: transition-region shape}
\label{sec:sparc_test}

The deep-MOND asymptotic behavior $\mu(x) \to \sqrt{x}$ as
$x \to 0$ is an observational input (the BTFR).  The Newtonian
limit $\mu \to 1$ as $x \to \infty$ is a consistency requirement.
The \emph{transition region} $0.1 \lesssim x \lesssim 10$, where
the interpolating function transitions between these regimes, is
where different proposals make distinct, testable predictions.

For the Petz interpolating function $\mu_{\mathrm{Petz}}(x) = 1 - e^{-\sqrt{x}}$,
the transition is characterized by $\mu(1) = 1 - e^{-1} \approx 0.632$
and $\mu'(1) = e^{-1}/(2) \approx 0.184$.  For comparison:
\begin{center}
\begin{tabular}{lcc}
\hline
Function & $\mu(1)$ & $\mu'(1)$ \\
\hline
$\mu_{\mathrm{Petz}} = 1 - e^{-\sqrt{x}}$ & 0.632 & 0.184 \\
$\mu_{\mathrm{simple}} = x/(1+x)$ & 0.500 & 0.250 \\
$\mu_{\mathrm{standard}} = x/\sqrt{1+x^2}$ & 0.707 & 0.354 \\
$\mu_{\mathrm{McGaugh}} = 1 - e^{-\sqrt{x}}$ & 0.632 & 0.184 \\
\hline
\end{tabular}
\end{center}
The Petz function is \emph{identical} to the empirical fit
proposed by McGaugh et al.~\cite{McGaugh2016RAR} based on 2693
data points from the SPARC sample.  This is a non-trivial
coincidence: the function was derived here from the Petz
recovery bound with BTFR as boundary condition, while McGaugh
obtained it as a purely empirical best fit.

\emph{Quantitative test.}---A $\chi^2$ fit of $\mu_{\mathrm{Petz}}$
against the full SPARC 175-galaxy RAR dataset~\cite{Lelli2017}
would determine whether the zero-parameter Petz prediction
($\Sigma = 2\sqrt{x}$ at all $x$) is sufficient, or whether
subleading corrections ($\Sigma = 2\sqrt{x} + \alpha x + \cdots$)
are required.  A finding of $\alpha = 0$ within uncertainties
would confirm the minimal Petz ansatz; $\alpha \neq 0$ would
indicate additional structure in the galactic entropy production
beyond the leading Petz bound.
...
\item[\textsc{A3.}] $J(Y)$ functional form: free function,
  ...
  interpolation~\eqref{eq:crooks_mu} is an ansatz inspired by
  thermodynamic fluctuation relations; a rigorous derivation
  from Crooks theorem remains open;
...
\item[\textsc{A7.}] Decoupling: the galactic sector $J(Y)$ and
  cosmological sector $K(Q)$ have no cross-coupling terms in the
  action.  This is a simplification; cross-terms may be needed
...
DESI DR2 and Euclid will directly probe this.  Conversely,
the continued null results of direct detection experiments
(LZ~\cite{LZ2024}, XENONnT~\cite{XENON2023}) are a \emph{prediction} of this
framework: no dark matter particle exists to be found.
...
\begin{proof}[Proof sketch]
The value $\delta(a_{\mathrm{rec}})$ is a non-negative real number,
so exactly three regimes are possible.

\emph{Case~1: $\delta(a_{\mathrm{rec}}) \ll 1$.}---Condition~(ii)
requires $\delta(a_{\mathrm{rec}}) < 2\times10^{-6}$.
Since $\mu^2\delta = I_0/(2a^3)$, the energy density
$\rho_K = c^2 I_0(2{+}\delta)/(16\pi G\,a^3)$
deviates from $a^{-3}$ by the exact fraction
\begin{equation}
\Delta(a) = \frac{2+\delta(a)}{2+\delta_0} - 1.
\label{eq:Delta_sketch}
\end{equation}
At recombination, $\delta \approx 0$, so
$|\Delta| = \delta_0/(2{+}\delta_0)$.
With $\delta_0 = 0.34$ from condition~(i):
$|\Delta| = 0.34/2.34 = 14.5\%$, violating condition~(iii).
This bound is independent of $\mu(a)$.

\emph{Cases~2 and~3: $\delta(a_{\mathrm{rec}}) \gtrsim O(1)$.}---The
sound speed $\cs^2 = \delta/(2{+}3\delta)$
gives $\cs^2 \geq 0.1$ for $\delta \gtrsim 0.25$, violating
condition~(ii) by $\geq 5$ orders of magnitude.  For
$\delta \gg 1$, the saturation $\cs^2 \to 1/3$ is structural
(see Supplemental Material, Theorem~2).
No choice of $K(Q)$ avoids this: all analytic $K$ with
$K''(1) > 0$ give $\cs^2 = O(1)$ when $\delta = O(1)$.

No fine-tuned $\mu(a)$ can avoid this trichotomy.
See Supplemental Material for the complete proof.
\end{proof}
...
Blanchet \& Skordis~\cite{BS2025} showed that adding a
...
acceleration scale $a_0 \approx cH_0/(2\pi) = 1.04 \times 10^{-10}\;\mathrm{m/s}^2$,
within 13\% of the empirical value (Sec.~\ref{sec:a0_mu}).
...
$\mu(x)$ via Crooks).  The transition scale $a_0 \approx cH_0/(2\pi)$
is suggestive but unproven (Sec.~\ref{sec:a0_mu}).
...
\bibitem{PaperIII_petz_interp}
S.-K.~Huang,
``Supplement to Paper~III: Petz recovery bound and the
MOND interpolating function,''
companion supplement (2026).
...
S.-K.~Huang,
``Supplementary material: Stability analysis of the Khronon
condensate,''
companion file \texttt{paper3\_stability.tex} (2026).
