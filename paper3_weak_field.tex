...
\emph{Completeness.}---The value $\delta(a_{\mathrm{rec}})$ is a
non-negative real number, so exactly three regimes are possible:
$\delta \ll 1$, $\delta \gg 1$, or $\delta \sim O(1)$.  In the
first case the $a^{-3}$ dilution requirement forces
$\delta(a_{\mathrm{rec}})$ too small for the observed
$\Omega_{\mathrm{DM}}$; in the second and third cases
$\cs^2 \geq O(0.1)$, violating condition~(ii).  No fine-tuned
$\mu(a)$ can avoid this trichotomy: regardless of the path
$\mu(a)$ takes, $\delta(a_{\mathrm{rec}})$ must fall in one of
these three regimes, and each violates at least one condition.
