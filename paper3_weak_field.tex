...
\end{enumerate}
The $\Sigma = 2\ln Q$ framework provides partial answers to all
three: $\mu_0 = H_0/c$ is the unique scale without Planck physics
(question~1); the no-go theorem forces $\cs^2 = 0$ via the tensor
field, making the CMB \emph{necessarily} $\Lambda$CDM-like
(question~2); and the Petz recovery bound suggests
$\mu(x) = 1 - e^{-\sqrt{x}}$ as the unique form consistent with
$\tauasym = 1 - e^{-\Sigma/2}$ (question~3, see
Supplement~\cite{PaperIII_petz_interp}).
