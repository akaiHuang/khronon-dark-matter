...
\begin{abstract}
We derive the MOND interpolating function
$\mu(x) = 1 - e^{-\sqrt{x}}$ directly from the Petz recovery
fidelity bound $F \geq e^{-\Sigma/2}$ established in Paper~1,
with the identification $\Sigma_{\mathrm{gal}} = 2\sqrt{x}$
where $x = g_{\mathrm{bar}}/a_0$. This provides an
information-theoretic origin for the transition between
Newtonian and deep-MOND regimes: the interpolating function
\emph{is} the Petz recovery failure $\tauasym = 1 - F$.
We verify that both asymptotic limits are correctly recovered,
compare with standard interpolating functions used in the
literature, and identify the remaining conjecture that requires
...
\assumed\ We identify the galactic gravitational entropy production as
\begin{equation}
...
The derivation rests on one non-trivial identification:
\begin{equation}
  \Sigma_{\mathrm{gal}} = 2\sqrt{x}\,.
  \tag{\ref{eq:sigma-gal}}
\end{equation}
This is \assumed, not \derived. We offer three supporting arguments:
...
\textbf{(ii) Asymptotic uniqueness.}
Consider the general ansatz $\Sigma_{\mathrm{gal}} = 2\,x^\alpha$.
...
$\mu_0$ that governs the cosmological sector. This suggests a
unified microscopic origin, though the derivation of
$\Sigma_{\mathrm{gal}} = 2\sqrt{x}$ from first principles remains
an open problem.
