...
This clarifies the role of the Blanchet--Skordis
...
$\mu(a)$ through Eq.~\eqref{eq:I0}.  We consider three
regimes for $\delta(a_{\mathrm{rec}})$.
...
We now show that condition~(3) is violated for
\emph{any} $\mu(a)$ achieving this bound, by an exact,
model-independent calculation.

From the conservation law
$\mu^2\delta = I_0/(2a^3)$,
the energy density (Eq.~\eqref{eq:rhoK} of the main text)
takes the form
\begin{equation}
\rho_K(a) = \frac{c^2 I_0}{16\pi G}\,
\frac{2+\delta(a)}{a^3}.
\label{eq:rhoK_I0}
\end{equation}
For exact CDM scaling ($\rho \propto a^{-3}$), the
factor $(2{+}\delta)$ must be constant.
Normalizing to $a_0 = 1$, the fractional deviation from
$a^{-3}$ at any epoch~$a$ is
\begin{equation}
\Delta(a) \;\equiv\;
\frac{\rho_K(a)}{\rho_K(a_0)\,(a_0/a)^3} - 1
\;=\; \frac{2+\delta(a)}{2+\delta_0} - 1.
\label{eq:Delta_def}
\end{equation}
This expression is exact and depends on $\mu(a)$
only through $\delta(a)$.

Now evaluate at $a = a_{\mathrm{rec}}$.
Since $0 \leq \delta(a_{\mathrm{rec}}) < 2\times10^{-6}$:
\begin{equation}
\Delta(a_{\mathrm{rec}})
= \frac{2 + \delta(a_{\mathrm{rec}})}{2 + \delta_0} - 1
\;\in\;\left[\frac{2}{2{+}\delta_0} - 1,\;\;
\frac{2{+}2\times10^{-6}}{2{+}\delta_0} - 1\right].
\label{eq:Delta_bound}
\end{equation}
With $\delta_0 = 0.34$ (fixed by condition~(1)):
\begin{equation}
\Delta(a_{\mathrm{rec}})
= \frac{2}{2.34} - 1 + O(10^{-6})
= -\frac{\delta_0}{2+\delta_0} + O(10^{-6})
= -0.1453.
\label{eq:Delta_exact}
\end{equation}
That is, the energy density at recombination is
$14.5\%$ \emph{below} the $a^{-3}$ extrapolation from
today---regardless of how $\mu(a)$ varies.  This
exceeds the $1\%$ tolerance of condition~(3) by more
than an order of magnitude.

\emph{Model-independence.}---The bound~\eqref{eq:Delta_exact}
uses only two ingredients:
(a)~the identity $\rho_K \propto (2{+}\delta)/a^3$, which
follows from the conservation law and is independent of
$\mu(a)$; and
(b)~the boundary values $\delta_0 = 0.34$ (condition~1)
and $\delta_{\mathrm{rec}} < 2\times10^{-6}$
(condition~2 in this regime).
No assumption about the functional form of $\mu(a)$ or
the intermediate trajectory $\delta(a)$ is needed.
The deviation
$|\Delta| = \delta_0/(2{+}\delta_0)$ coincides
with the equation of state $w_0 = \delta_0/(2{+}\delta_0)
= 0.145$, which is the irreducible cost of matching
$\Omega_{\mathrm{DM}}$ with a non-constant $(2{+}\delta)$
factor.
...
\emph{Completeness.}---The three cases are exhaustive because
$\delta(a_{\mathrm{rec}})$ is a non-negative real number.
Case~1 ($\delta \ll 1$), Case~2 ($\delta \gg 1$), and Case~3
($\delta \sim O(1)$) cover the entire range.  No fine-tuned
$\mu(a)$ can avoid this trichotomy: regardless of the path
$\mu(a)$ takes, the value $\delta(a_{\mathrm{rec}})$ must fall
in one of these three regimes, and each violates at least one
condition.
...
Blanchet--Skordis tensor field~\cite{BS2025} from a
...
Blanchet--Skordis (2025) tensor field---or an
